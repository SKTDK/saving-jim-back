...
router.post("/addUser", function (req, res, next) {
    ...
    // Checking if data is valid before sending it to remote DB server
    if (
        !firstname.match(config.REGEX_NAME) ||
        firstname.length > config.LEN_NAME ||
        !lastname.match(config.REGEX_NAME) ||
        lastname.length > config.LEN_NAME ||
        !password.match(config.REGEX_PASSWORD) ||
        password.length > config.LEN_PASSWORD ||
        !username.match(config.REGEX_USERNAME) ||
        username.length > config.LEN_USERNAME
    ) {
        res.status(413).json({
            success: false,
    ...
    bcrypt.hash(password, saltRounds, function (err, hash) {
        if (err) {
            ...
            db.db
                .query(
                    "INSERT INTO savingjim.users (id, account_type, first_name, last_name, username, password, active, modified_on, modified_by, version) VALUES (DEFAULT, $1, $2, $3, $4, $5, true, NOW(), NULL, 1)",
                    [accountType, firstname, lastname, username, password]
                )
                .then(result => {
                    if (result) {
                        res.status(200).json({
...
router.post("/changeAccountState", function (req, res, next) {
    ...
    if (payload.user.account_type === 0) {
        ...
        db.db
            .query("UPDATE savingjim.users SET active=$1 WHERE username=$2", [
                active,
                username
            ])
            .then(result => {
...
router.post("/updateAccount", function (req, res, next) {
    ...
    // users are not able to update anything
    if (payload.user.account_type === 2 || payload.user.account_type === 3) {
        res.status(412).json({
            success: false,
            error: "Not allowed"
    ...
    // A manager cannot update an admin or another manager
    if (payload.user.account_type === 1 && (accountType === 0 || accountType === 1)) {
        res.status(412).json({
            success: false,
            error: "Not allowed"
        });
    ...
    if (firstname && lastname && password) {
        ...
        bcrypt.hash(password, saltRounds, function (err, hash) {
            if (err) {
                ...
                db.db
                    .query(
                        "UPDATE savingjim.users SET first_name=$1, last_name=$2, password=$3 WHERE id=$4",
    ...
    if (firstname && lastname) {
        ...
        db.db
            .query(
                "UPDATE savingjim.users SET first_name=$1, last_name=$2 WHERE id=$3",
    ...
    if (firstname && password) {
        ...
        bcrypt.hash(password, saltRounds, function (err, hash) {
            if (err) {
                ...
                db.db
                    .query(
                        "UPDATE savingjim.users SET first_name=$1, password=$2 WHERE id=$3",
                        [firstname, password, id]
                    )
                    .then(result => {
                        if (result) {
                            res.status(200).json({
    ...
    if (lastname && password) {
        ...
        bcrypt.hash(password, saltRounds, function (err, hash) {
            if (err) {
                ...
                db.db
                    .query(
                        "UPDATE savingjim.users SET last_name=$1, password=$2 WHERE id=$3",
    ...
    if (firstname) {
        ...
        db.db
            .query("UPDATE savingjim.users SET first_name=$1 WHERE id=$2", [
                firstname,
                id
            ])
            .then(result => {
    ...
    if (lastname) {
        ...
        db.db
            .query("UPDATE savingjim.users SET last_name=$1 WHERE id=$2", [
                lastname,
                id
            ])
            .then(result => {
    ...
    if (password) {
        ...
        bcrypt.hash(password, saltRounds, function (err, hash) {
            if (err) {
                ...
                // Insert user into DB
                db.db
                    .query("UPDATE savingjim.users SET password=$1 WHERE id=$2", [
                        password,
                        id
                    ])
                    .then(result => {
...
router.post("/search", function (req, res, next) {
    ...
    if (
        !text.match(config.REGEX_USERNAME) ||
        text.length > config.LEN_USERNAME
    ) {
        res.status(413).json({
            success: false,
            error: "Wrong data sent! (2)"
...
router.post("/searchByUser", function (req, res, next) {
    ...
    db.db
        .query(
            "SELECT DISTINCT u.id, u.account_type, u.first_name, u.last_name, u.username, u.active, u.modified_on, u.modified_by, u.version FROM savingjim.users u, savingjim.persons_of_contact poc WHERE u.account_type=$1 AND poc.child_id=$2 LIMIT 100",
